...
TEST(Memory, BasicMemory) {
    ...
    wram.Write(defs::mmu_addresses::kWramStart + 2, 42);
    EXPECT_EQ(wram.Read(defs::mmu_addresses::kWramStart + 2), 42);

    vram.Write(defs::mmu_addresses::kVramStart + 42, 67);
    EXPECT_EQ(vram.Read(defs::mmu_addresses::kVramStart + 42), 67);
}
